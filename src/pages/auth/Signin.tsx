    ...
    <section className='bg-primary-blue min-h-screen bg-no-repeat w-full bg-center bg-contain flex items-center justify-center' style={{'backgroundImage':`url(${background})`}}>
        <aside className='w-[599px] '>
            ...
            <div className='py-10 px-14 bg-white flex flex-col gap-5 rounded-b-lg'>
                <article>
                    <h2 className='text-black text-3xl leading-10 font-medium'>Sign in</h2>
      ...
      <input 
        ...
        className={`rounded-lg p-3 placeholder-[#A8A8A8)] ${!isValidEmail && email ? 'bg-light-red border border-danger-red' : 'bg-[#F4F4F4]'}`}
      />
      {!isValidEmail && email && <p className='text-red-500 text-xs'>Not a valid email address</p>}
    </div>
                <div className='flex flex-col gap-2'>
                ...
                <p className='text-center text-sm flex justify-center gap-1'>Don’t have an account? <Link to='/signup' className='text-[#0C4C84]'>Sign up here</Link> <span><img src={arrowRight} alt="arrow" /></span></p>
            </div>
        </aside>
    </section>
  )
